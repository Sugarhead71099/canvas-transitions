...
window.onload = function() {
	...
	picturePixelWidth = canvas.width / 20, picturePixelHeight = canvas.height / 20;

	// Add new image for every image that you want to display
	newImage(picturePixelWidth, picturePixelHeight, "Images/logo.png");
	newImage(picturePixelWidth, picturePixelHeight, "Images/logo.jpg");
	newImage(picturePixelWidth, picturePixelHeight, "Images/taxDollars.jpg");
	newImage(picturePixelWidth, picturePixelHeight, "Images/visa.jpg");
	newImage(picturePixelWidth, picturePixelHeight, "Images/americanFlag.jpg");
	newImage(picturePixelWidth, picturePixelHeight, "Images/gunControl.jpg");
	newImage(picturePixelWidth, picturePixelHeight, "Images/nSA.jpg");
	newImage(picturePixelWidth, picturePixelHeight, "Images/candidateTalking.jpg");
	...
	setTimeout(function() {cyclePictures(pictures[imageCounter], images[imageCounter]);}, 1500);
	timer = setInterval(function() {cyclePictures(pictures[imageCounter], images[imageCounter]);}, 4000);
}
...
function newImage(width, height, name) {
    ...
    var ImageObject = {
        ...
        h: height,
        draw: function() {
            canvasContext.drawImage(image, this.sx, this.sy, this.sx + this.sw, this.sy + this.sh, this.x, this.y, this.x + this.w + 1, this.y + this.h + 1);
        }
    };
			...
			timer2 = setInterval(function () {
				if(image.complete) {
					if(cycleStarted === true)
						canvasContext.clearRect(ImageObject.x, ImageObject.y, ImageObject.x + ImageObject.w, ImageObject.y + ImageObject.h);
					ImageObject.draw();
		...
		multiObjectOuterEdgesToCenterX : function(ImageObject, image) {
			...
			timer2 = setInterval(function () {
				if(image.complete) {
					if(cycleStarted === true) {
						canvasContext.clearRect(ImageObject.x, ImageObject.y, ImageObject.x + ImageObject.w, ImageObject.y + ImageObject.h);
						canvasContext.clearRect(ImageObject2.x, ImageObject2.y, ImageObject2.x + ImageObject2.w, ImageObject2.y + ImageObject2.h);
					}
					ImageObject.draw();
					ImageObject2.draw();
...
function radioButtonAction(buttonClickedId) {
	switch(buttonClickedId) {
		...
		case document.getElementById("picture4"):
			...
			imageCounter = 4;
			cyclePictures(pictures[imageCounter], images[imageCounter]);
			timer = setInterval(function() {pictureSelections[imageCounter].checked = true; cyclePictures(pictures[imageCounter], images[imageCounter]);}, 4000);
			break;
